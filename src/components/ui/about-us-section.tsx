...
import { Youtube, Sword, Users, Calendar, TrendingUp, Disc, Play, Sparkles } from "lucide-react";
import {
  motion,
  useScroll,
  useTransform,
      ...
      >
        {loading && (!fetchedServices || fetchedServices.length === 0) ? (
          <div className="flex flex-col items-center justify-center min-h-[400px]">
            ...
            <p className="text-accent-purple font-black text-xs uppercase tracking-widest">
              Summoning Services...
            </p>
          </div>
        ) : (
          <>
            <motion.div className="flex flex-col items-center mb-6" variants={itemVariants}>
              <motion.span className="text-accent-purple font-black mb-2 flex items-center gap-2 tracking-[0.2em] text-xs">
                <Sword className="w-4 h-4" />
                DOMINATE THE DIGITAL REALM
              </motion.span>
              <h2 className="text-4xl md:text-5xl font-black mb-4 text-center uppercase tracking-tighter">
                Digital Sovereignty
              </h2>
              <motion.div
            ...
            <motion.p
              className="text-center max-w-2xl mx-auto mb-16 text-deep-purple/70 text-lg md:text-xl italic font-medium leading-relaxed"
              variants={itemVariants}
            >
              "We build unshakeable digital fortresses and interactive kingdoms tailored to your
...
function ServiceItem({
  ...
  return (
    ...
    >
      ...
      >
        <motion.div className="text-deep-purple bg-accent-yellow p-4 rounded-2xl transition-all duration-300 group-hover:bg-black group-hover:text-white group-hover:rotate-[10deg] shadow-lg relative">
          {icon}
...
function StatCounter({
  ...
  return (
    <motion.div
      className={cn(
        "bg-white/50 backdrop-blur-md p-8 rounded-3xl flex flex-col items-center text-center group hover:bg-white transition-all duration-300 border border-accent-purple/5 shadow-xl",
        link ? "cursor-pointer" : "",
      )}
      variants={
        {
    ...
    >
      ...
      <motion.div
        ref={countRef}
        className="text-4xl font-black text-deep-purple flex items-center tracking-tighter"
      >
        {stringValue ? (
      ...
      <p className="text-deep-purple/70 text-xs font-bold uppercase tracking-widest mt-2">
        {label}
      </p>
