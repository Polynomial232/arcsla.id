    ...
    <div className="relative w-full mt-10 overflow-hidden group/carousel" ref={containerRef}>
      ...
      >
        {totalItems.map((item, index) => (
          <div key={`card-${index}`} className="flex-none rounded-[2.5rem] pointer-events-none">
            {item}
          </div>
        ))}
      </motion.div>
...
const TestimonialCard = ({ testimonial, index }: { testimonial: iTestimonial; index: number }) => {
  return (
    <div
      className={cn(
